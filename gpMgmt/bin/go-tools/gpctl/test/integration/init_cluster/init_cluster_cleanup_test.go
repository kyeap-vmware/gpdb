...
var expectedRollbackMsg = "[INFO]:-Please run gpctl init --clean to rollback"
...
func TestInitClusterCleanup(t *testing.T) {
	t.Run("check if the coordinator is stopped whenever an error occurs", func(t *testing.T) {
		...
		if !strings.Contains(result.OutputMsg, expectedRollbackMsg) {
			t.Fatalf("got %q, want %q", result.OutputMsg, expectedRollbackMsg)
		}
	...
	t.Run("cluster creation fails when mirror is getting created, test if rollback is successful", func(t *testing.T) {
		...
		// Verify there are no postgres processes running in coordinator and primary
