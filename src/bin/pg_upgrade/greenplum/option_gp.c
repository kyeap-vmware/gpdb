...
initialize_greenplum_user_options(void)
{
	...
	greenplum_user_opts.migrate_checks = false;
}
...
process_greenplum_option(greenplumOption option)
{
	switch (option)
	{
		...
		case GREENPLUM_MIGRATE_CHECKS:
			greenplum_user_opts.migrate_checks = true;
			break;
...
{
	return greenplum_user_opts.skip_target_check || greenplum_user_opts.migrate_checks;
}
...
migrate_checks(void)
{
	return greenplum_user_opts.migrate_checks;
}
