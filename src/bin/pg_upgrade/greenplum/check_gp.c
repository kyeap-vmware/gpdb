...
check_views_with_removed_columns()
{
	...
	snprintf(output_path, sizeof(output_path), "%s/%s",
			 log_opts.basedir, "views_with_removed_columns.txt");
	...
	for (dbnum = 0; dbnum < old_cluster.dbarr.ndbs; dbnum++)
	{
		...
		res = executeQueryOrDie(conn,
								...
								"FROM pg_class c JOIN pg_namespace n on c.relnamespace=n.oid "
								"WHERE c.relkind = 'v' AND "
								"view_has_removed_columns(c.oid) = TRUE AND "
								"n.nspname NOT IN ('pg_catalog', 'gp_toolkit', 'information_schema');");

		PQclear(executeQueryOrDie(conn, "DROP FUNCTION view_has_removed_columns(OID);"));
		PQclear(executeQueryOrDie(conn, "SET search_path to 'pg_catalog';"));
		PQclear(executeQueryOrDie(conn, "RESET track_counts;"));
	...
	if (script)
	{
		...
		gp_fatal_log(
			"| Your installation contains views using\n"
			"| columns from catalog tables that have been removed.\n"
...
/* checks for views with removed tables, views, and materialized views */
static void
check_views_with_removed_tables()
{
	if (GET_MAJOR_VERSION(old_cluster.major_version) > 904)
	...
	prep_status("Checking for views with removed tables");

	snprintf(output_path, sizeof(output_path), "%s/%s",
			 log_opts.basedir, "views_with_removed_tables.txt");
	...
	for (dbnum = 0; dbnum < old_cluster.dbarr.ndbs; dbnum++)
	{
		...
		res = executeQueryOrDie(conn,
								...
								"WHERE c.relkind = 'v' "
								"AND c.oid >= 16384 "
								"AND view_has_removed_tables(c.oid) = TRUE;");

		PQclear(executeQueryOrDie(conn, "DROP FUNCTION view_has_removed_tables(OID);"));
		PQclear(executeQueryOrDie(conn, "SET search_path to 'pg_catalog';"));
		PQclear(executeQueryOrDie(conn, "RESET track_counts;"));
	...
	if (script)
	{
		...
		gp_fatal_log(
			"| Your installation contains views using\n"
			"| catalog tables that have been removed.\n"
