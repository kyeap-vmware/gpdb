...
typedef enum {
	...
	GREENPLUM_NOT_IN_PLACE_UPGRADE = 16
} greenplumOption;
...
#define GREENPLUM_OPTIONS \
	...
	{"not-in-place-upgrade", no_argument, NULL, GREENPLUM_NOT_IN_PLACE_UPGRADE},
#define GREENPLUM_USAGE "\
  ...
  --not-in-place-checks         only run subset of checks needed for upgrading using gpbackp.\"\n\
"
...
bool not_in_place_upgrade(void);
