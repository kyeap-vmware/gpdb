...
setup_connection(Archive *AH, const char *dumpencoding, char *use_role)
{
	 ...
	 */
	if (quote_all_identifiers && AH->remoteVersion >= 80323)
...
getTables(Archive *fout, int *numTables)
{
	 ...
	 */
	if (fout->remoteVersion >= GPDB6_PGVERSION)
	 		appendPQExpBufferStr(query,
...
getBMIndxInfo(Archive *fout)
{
	...
	bitmap_index_namespace = fout->remoteVersion >= GPDB6_PGVERSION ? PG_BITMAPINDEX_NAMESPACE : 3012;
